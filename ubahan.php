...
<body class="bg-gray-100 flex">
    <aside class="w-64 h-screen bg-grey p-6 shadow fixed sidebar">
        ...
        <nav class="flex flex-col space-y-5">
            ...
            <a href="ubahan.php" class="sidebar-link active">🗃️<span>Bahan</span></a>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white rounded-lg shadow product-table">
                <thead class="table-header">
                    <tr>
                        <th class="table-cell">Nama bahan</th>
                        <th class="table-cell">Harga</th>
                        <th class="table-cell"> Stok </th>
                        <th class="table-cell">Nama Suppliyer</th>
                    </tr>
                </thead>
                <tbody class="text-gray-700 divide-y divide-gray-200">
                    <?php if (mysqli_num_rows($result) > 0): ?>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                <td class="table-cell"><?= htmlspecialchars($row['nama_bahan']) ?></td>
                                <td class="table-cell">Rp <?= number_format($row['harga'], 0, ',', '.') ?></td>
                                <td class="table-cell"><?= $row['stok_bahan'] ?></td>
                                 <td class="table-cell"><?= htmlspecialchars($row['nama_suppliyer']) ?></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
